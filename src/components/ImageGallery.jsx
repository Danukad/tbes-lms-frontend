...
const ImageGallery = () => {
    ...
    return (<div className="bg-white flex flex-col items-center min-h-screen overflow-hidden relative">
            <div className="relative">
                <h1 className="text-black text-[40px] font-bold absolute top-[40px] right-[440px]">Gallery</h1>
            </div>

            <img src={background} alt="Decorative Graphic" className="absolute top-0 right-[-150px] w-[850px] pointer-events-none z-0 md:right-[665px]"/>
            <div className="w-full max-w-[1200px] px-6 flex-1 relative top-[200px]">
                <div className="relative">
                    <div className="overflow-hidden ">
                        <div className="flex animate-scroll space-x-4 ">
                            {/* Render photos twice for seamless looping */}
                            {[...photos, ...photos].map((photo, index) => (<div key={`${photo.id}-${index}`} className="min-w-[460px] h-auto aspect-square rounded-lg overflow-hidden hover:paused">
                                    <img src={photo.src} alt={photo.alt} className="max-w-[450px] w-full h-auto object-contain"/>
                                </div>))}
                        </div>
                    </div>
                    <button onClick={handleShowMore} className="absolute top-[-60px] right-0 text-black text-[20px] px-4 py-2 rounded-md hover:text-[#364CE2] transition-colors">
                        Show More
                    </button>
                    <style>
                        {`
...
const FullGallery = () => {
    ...
    return (<div className="bg-white flex flex-col items-center overflow-hidden">
            ...
            <div className="w-full max-w-[1200px] px-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 flex-1">
                {galleryPhotos.length > 0 ? (galleryPhotos.map((photo) => (<div key={photo.id} className="w-full aspect-auto bg-gray-200 rounded-lg overflow-hidden">
                            <img src={photo.src} alt={photo.alt} className="w-full object-cover transition-transform duration-500 ease-in-out transform hover:scale-110"/>
                        </div>))) : (<p className="text-center text-gray-500 font-inter">No photos available.</p>)}
            </div>
        </div>);
};
