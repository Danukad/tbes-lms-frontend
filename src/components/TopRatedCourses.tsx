...
const TopRatedCourses: React.FC = () => {
    const navigate = useNavigate();
    const [courses, setCourses] = useState(coursesData);
    const [newCourseAnimation, setNewCourseAnimation] = useState<number[]>([]);
    ...
    return (
        <div className="bg-[#ebedf0] min-h-screen flex flex-col items-center py-16 overflow-hidden">
                ...
                {courses.map((course) => (
                    <div
                        ...
                        className={`bg-white rounded-lg shadow-md overflow-hidden transition-all duration-300 ${
                            newCourseAnimation.includes(course.id) ? 'animate-new-course' : 'hover:shadow-lg'
                        }`}
                    >
                        <img
                            src={course.image}
                        ...
                        {newCourseAnimation.includes(course.id) && (
                            <style>
                                {`
                  @keyframes newCourse {
