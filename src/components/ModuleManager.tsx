...
const ModuleManager = () => {
    ...
    const [newContent, setNewContent] = useState({ title: '', type: '', description: '', tutorName: '', file: null as File | null });
    ...
    const handleAddModule = async () => {
        ...
        try {
            await axios.post(
                'http://192.168.8.130:5000/api/modules',
                { title: newModuleTitle, courseId: selectedCourseId },
                { headers: { Authorization: `Bearer ${token}` } }
            );
            setNewModuleTitle('');
            setModalOpen(false);
    ...
    const handleEditModule = async () => {
        ...
        try {
            await axios.put(
                `http://192.168.8.130:5000/api/modules/${currentModule._id}`,
    ...
    const handleAddContent = async (e: React.FormEvent) => {
        ...
        try {
            await axios.post(
                `http://192.168.8.130:5000/api/modules/content/${selectedModuleId}`,
                formData,
    ...
    const handleDeleteContent = async (contentId: string) => {
        ...
        try {
            await axios.delete(
                `http://192.168.8.130:5000/api/modules/content/${contentId}`,
                { headers: { Authorization: `Bearer ${token}` } }
            );
    ...
    return (
        <div className="p-6">
                ...
                <>
                    <div className="flex items-center justify-between">
                        ...
                        <button
                            ...
                            className="bg-[#C9A500] text-white px-3 py-1 rounded hover:bg-[#A78A00] mr-2"
                        >
                            + Add Module
                        </button>
                        <button
                    ...
                    <ul className="mb-4">
                        {modules.map((mod) => (
                            <li key={mod._id} className="flex justify-between items-center border p-2 rounded">
                                ...
                                <div>
                                    <button
                                        onClick={() => { setCurrentModule(mod); setNewModuleTitle(mod.title); setModalType('edit'); setModalOpen(true); }}
                                        className="text-sm text-blue-600 hover:underline mr-2"
                                    >
                                        Edit
                                    </button>
                                    <button
                                        onClick={() => { setCurrentModule(mod); setModalType('delete'); setModalOpen(true); }}
            ...
            {Array.isArray(contents) && contents.length > 0 && selectedModuleId && (
                <div className="mt-6">
                    <div className="flex items-center justify-between mb-2">
                        <h3 className="text-xl font-semibold text-[#364CE2]">Module Contents</h3>
                        <button
                            onClick={() => { setModalType('add'); setModalOpen(true); }}
                            className="bg-[#364CE2] text-white px-3 py-1 rounded hover:bg-[#283BBE]"
                        >
                            + Add Content
                        </button>
                    </div>
                    <ul className="space-y-2">
                        {contents.map((item) => (
                            <li key={item._id} className="p-3 border rounded shadow-sm flex justify-between">
                <span>
            ...
            {modalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white p-6 rounded-lg shadow-lg w-96">
                        ...
                        {modalType === 'add' && (
                            modalType === 'add' && selectedModuleId ? (
                                <form onSubmit={handleAddContent}>
                                    ...
                                    <input
                                        type="text"
                                        value={newContent.tutorName}
                                        onChange={(e) => setNewContent({ ...newContent, tutorName: e.target.value })}
                                    ...
                                    <div className="flex justify-end">
                                        <button
                                            type="button"
                                            onClick={() => { setModalOpen(false); setError(null); setNewContent({ title: '', type: '', description: '', tutorName: '', file: null }); }}
                                ...
                                <div>
                                    ...
                                    <div className="flex justify-end">
                                        <button
                                            type="button"
                                            onClick={() => { setModalOpen(false); setError(null); setNewModuleTitle(''); }}
                                            className="mr-2 text-gray-600 hover:text-gray-800"
                                        >
                                            Cancel
                                        </button>
                                        <button
                                            ...
                                            className="bg-[#C9A500] text-white px-3 py-1 rounded hover:bg-[#A78A00]"
                                        >
                                            Add
                                        </button>
                                    </div>
