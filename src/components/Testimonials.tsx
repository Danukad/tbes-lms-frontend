            ...
            return (
                <div className="text-center text-red-600 p-4">
                    ...
                    <button
                        ...
                        className="mt-2 bg-[#C9A500] text-black px-4 py-2 rounded-lg hover:bg-opacity-80"
                    >
                        Reload
                    </button>
                </div>
...
const Testimonials: React.FC = () => {
    ...
    const fetchReviews = async () => {
        ...
        try {
            ...
            setError(
                err.message === 'Network Error' ? 'Server unavailable, please try again later' : err.response?.data?.error || 'Failed to load reviews'
            );
        } finally {
            setLoading(false);
        }
    ...
    return (
        <ErrorBoundary>
            <div className="bg-gradient-to-b from-[#364CE2] via-[#010D60] to-[#02051B] min-h-[80vh] xs:min-h-[80vh] sm:min-h-[85vh] flex flex-col items-center py-6 xs:py-8 sm:py-12 overflow-hidden font-['Montserrat',sans-serif]">
                ...
                {(error || imageError) && (
                    <div className="bg-red-600 text-white p-3 xs:p-4 sm:p-4 rounded-lg mb-4 xs:mb-6 sm:mb-6 max-w-[90vw] xs:max-w-[260px] sm:max-w-[1000px] md:max-w-[1200px] w-full text-center text-xs xs:text-xs sm:text-sm">
                        {error || imageError}
                        {error && (
                            <button
                                onClick={fetchReviews}
                                className="ml-2 xs:ml-3 bg-[#C9A500] text-black px-2 xs:px-3 sm:px-4 py-1 xs:py-1.5 sm:py-2 rounded-lg hover:bg-opacity-80 text-xs xs:text-xs sm:text-sm"
                            >
                                Retry
                            </button>
                        )}
                ...
                <div className="w-full max-w-[90vw] xs:max-w-[260px] sm:max-w-[1000px] md:max-w-[1200px] text-center px-3 xs:px-4 sm:px-6">
                    {loading && (
                        <div className="bg-white bg-opacity-10 rounded-lg p-6 xs:p-8 sm:p-12 text-center text-white text-base xs:text-lg sm:text-xl">
                            Loading reviews...
                        </div>
                    )}
                    {!loading && reviews.length === 0 && !error ? (
                        <div className="bg-white bg-opacity-10 rounded-lg p-6 xs:p-8 sm:p-12 text-center text-white">
                            <p className="text-base xs:text-base sm:text-lg">No reviews yet. Be the first to share your experience!</p>
                        </div>
                    ) : (
                        <>
                            {window.Swiper && reviews.length > 0 ? (
                                <div className="swiper p-4 xs:p-4 sm:p-6">
                                    <div className="swiper-wrapper">
                                        {reviews.map((review) => (
                                            <div
                                                className="swiper-slide bg-white bg-opacity-10 rounded-lg p-6 xs:p-6 sm:p-8 transition-all duration-500 ease-in-out"
                                                key={review._id}
                                            >
                                                <div className="flex flex-col items-center text-center text-white">
                                                    <div className="w-20 xs:w-20 sm:w-24 h-20 xs:h-20 sm:h-24 mb-3 xs:mb-4 sm:mb-4">
                                                        <img
                                                            src={`${apiBase}${review.photo}`}
                                                            alt={`${review.fullName}'s photo`}
                                                    ...
                                                    <div className="flex justify-center mb-2 xs:mb-2 sm:mb-2">
                                                        {[...Array(5)].map((_, i) => (
                                                            <FaStar
                                                                key={i}
                                                                className={`h-4 xs:h-4 sm:h-5 w-4 xs:w-4 sm:w-5 ${i < review.rating ? "text-[#C9A500]" : "text-gray-400"}`}
                                                            />
                                                        ))}
                                                    </div>
                                ...
                                <div className="grid grid-cols-1 xs:grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 xs:gap-4 sm:gap-6">
                                    {reviews.map((review) => (
                                        <div
                                            className="bg-white bg-opacity-10 rounded-lg p-6 xs:p-6 sm:p-8 text-center text-white"
                                            key={review._id}
                                        >
                                            <img
                                                src={`${apiBase}${review.photo}`}
                                                alt={`${review.fullName}'s photo`}
                                                className="rounded-full w-20 xs:w-20 sm:w-24 h-20 xs:h-20 sm:h-24 mx-auto mb-3 xs:mb-4 sm:mb-4 object-cover"
                                                onError={(e) => {
                                                    console.error(`Failed to load image: ${apiBase}${review.photo}`);
                                                    e.currentTarget.src = "https://via.placeholder.com/80?text=Image+Not+Found";
                                                    setImageError("Some review images failed to load.");
