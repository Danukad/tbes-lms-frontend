...
const EnrollStudent: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await axios.post(
                'http://192.168.8.130:5000/api/enroll',
                {
                    firstName,
                    lastName,
                    email,
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-[#364CE2] to-[#010D60] text-white font-inter p-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-6 bg-white bg-opacity-10 p-8 rounded-xl shadow-lg">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input
                                ...
                                className="w-full p-2 rounded-md bg-gray-800 text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-[#C9A500]"
                                required
                            />
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            onChange={handleChange}
                            className="w-full p-2 rounded-md bg-gray-800 text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-[#C9A500]"
                            placeholder="Enter NIC if applicable"
                        />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        {selectedBatch && (
                            <p className="mt-2 text-sm text-gray-300">{selectedBatch.description}</p>
                        )}
                    </div>
                    <button
                        type="submit"
                        disabled={loading}
                        className={`w-full bg-[#C9A500] text-black p-3 rounded-md hover:bg-opacity-80 transition ${
