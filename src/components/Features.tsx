...
const features = [
    {
        ...
        icon: (
            <svg
                ...
                viewBox="0 0 20 20"
                fill="currentColor"
                className="w-8 h-8 xs:w-8 xs:h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 text-[#C9A500]"
            >
                <path
                    fillRule="evenodd"
...
const Features: React.FC = () => {
    return (
        <div className="relative bg-[#1B61EF]/10 flex items-center justify-center py-8 xs:py-10 sm:py-12 md:py-16 min-h-[80vh] lg:h-[610px] xs:min-h-[85vh]  sm:min-h-screen overflow-hidden">
            {/* Background Image */}
            <img
                src={BackgroundBall}
                alt="Background decorative ball"
                className="absolute top-[-10%] right-[0%] w-[1400px] xs:w-0 sm:w-[400px] md:w-[600px] lg:w-[1040px] lg:h-[680px] xl:w-[1550px] xl:h-auto xl:top-0 z-0 opacity-50 xs:opacity-50 sm:opacity-70 lg:opacity-100 pointer-events-none select-none"
            />
            ...
            <div className="relative z-10 flex flex-col xs:flex-col sm:flex-row w-full max-w-[90vw] xs:max-w-[92vw] sm:max-w-[1200px] lg:max-w-[1400px] px-3 xs:px-4 sm:px-6 md:px-8">
                ...
                <div className=" relative w-full xs:w-full sm:w-3/5 grid grid-cols-1 xs:grid-cols-1 sm:grid-cols-2 gap-4 xs:gap-4 sm:gap-6 ">
                    {features.map((feature) => (
                        <div
                            key={feature.id}
                            className="bg-[white]/40 backdrop-blur-[8px] xs:backdrop-blur-[10px] border border-white/20 rounded-xl shadow-md p-4 xs:p-4 sm:p-5 md:p-6 lg:h-[170px] flex flex-col items-center text-center transition-all duration-300"
                        >
