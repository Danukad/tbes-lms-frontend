    ...
    return (<>
            <div className="min-h-[100vh] xs:min-h-[80vh] sm:min-h-[85vh] flex flex-col items-center font-['Montserrat',sans-serif] py-4 xs:py-4 sm:py-6 px-3 xs:px-3 sm:px-4">
                <div className="w-full max-w-[90vw] xs:max-w-[92vw] sm:max-w-[1000px] md:max-w-[1200px] bg-white rounded-xl shadow-2xl p-4 xs:p-4 sm:p-6 md:p-8 absolute top-28">
                    ...
                    {(error || imageError) && (<div className="bg-red-100 text-red-700 p-3 xs:p-4 rounded-lg mb-4 xs:mb-6 text-xs xs:text-xs sm:text-sm text-center max-w-[260px] xs:max-w-[260px] sm:max-w-[400px] mx-auto">
                            {error || imageError}
                            {error && (<button onClick={() => fetchCourses()} className="ml-2 xs:ml-3 bg-[#C9A500] text-black px-2 xs:px-3 py-1 xs:py-1.5 rounded-md hover:bg-[#A78A00] text-xs xs:text-xs sm:text-sm mt-2 xs:mt-0">
                                    Retry
                                </button>)}
                        </div>)}
                    <>
            ...
            return (<div key={course._id} className="border rounded-lg p-3 xs:p-4 bg-gray-50 flex flex-col">
                        ...
                        setImageError('Some course images failed to load.');
                    }} onLoad={() => console.log(`Loaded image: ${IMAGE_BASE_URL}/uploads/courses/${imagePath}`)}/>) : (<img src={`https://placehold.co/300x200?text=${encodeURIComponent(course.title)}`} alt={course.title} className="w-full h-[150px] xs:h-[150px] sm:h-[200px] object-contain rounded-md mb-2 xs:mb-3"/>)}
                                        ...
                                        <div className="flex gap-2 xs:gap-3 mt-auto">
                                            <button onClick={() => handleBuyNow(course._id)} className="flex-1 bg-[#C9A500] text-black px-2 xs:px-3 py-1 xs:py-1.5 rounded-md hover:bg-[#A78A00] text-xs xs:text-xs sm:text-sm">
                                                Buy Now
                                            </button>
                                            <button onClick={() => handleViewCourse(course._id)} className="flex-1 bg-gray-600 text-white px-2 xs:px-3 py-1 xs:py-1.5 rounded-md hover:bg-gray-700 text-xs xs:text-xs sm:text-sm">
                                                View Course
                                            </button>
                                        </div>
                                    </div>);
