...
const StudentDashboard = () => {
    ...
    const Marks = () => {
        ...
        if (error)
            return <p className="p-4 text-red-600">Error loading marks: {error.includes('404') ? 'Endpoint not found. Please check the server.' : error}. Try again later.</p>;
        if (marks.length === 0)
            return <p className="p-4 text-gray-600">Loading marks...</p>;
        return (<div className="p-4">
          ...
          <div className="space-y-6">
            <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
              <table className="w-full text-left">
                ...
                <tbody>
                {marks.map((mark, index) => (<tr key={index} className="hover:bg-gray-50 transition-colors duration-200">
    ...
    const Messages = () => {
        ...
        if (error)
            return <p className="p-4 text-red-600">Error loading messages: {error.includes('404') ? 'Endpoint not found. Please check the server.' : error}. Try again later.</p>;
          ...
          <div className="space-y-6">
            <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
              <ul className="space-y-2">
                {messages.map((msg, index) => (<li key={index} className="flex justify-between items-center p-2 hover:bg-gray-50 rounded transition-colors duration-200">
                      <div>
    ...
    const AcademicCalendar = () => {
        ...
        if (error)
            return <p className="p-4 text-red-600">Error loading calendar: {error.includes('404') ? 'Endpoint not found. Please check the server.' : error}. Try again later.</p>;
        if (!calendar)
            return <p className="p-4 text-gray-600">Loading calendar...</p>;
        return (<div className="p-4">
          ...
          <div className="space-y-6">
            <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
              ...
              <div className="grid grid-cols-7 gap-1 text-center text-gray-600">
                ...
                <div>Fri</div>
                <div>Sat</div>
                {Array.from({ length: 31 }, (_, i) => (<div key={i} className={`p-2 hover:bg-[#C9A500] hover:text-white rounded transition-colors duration-200 ${[6, 13, 20, 27].includes(i) ? 'bg-[#364CE2] text-white' : ''}`}>
                      {i + 1}
                    </div>))}
              </div>
            </div>
            <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
              ...
              <ul className="space-y-2">
                {calendar.events && calendar.events.length > 0 ? (calendar.events.map((event, index) => (<li key={index} className="flex justify-between text-gray-700">
                          <span>{event.title || 'N/A'}</span>
                          <span className="text-[#364CE2]">{event.date ? new Date(event.date).toLocaleDateString() : 'N/A'}</span>
                        </li>))) : (<li className="p-2 text-gray-600">No events available</li>)}
              </ul>
            </div>
          </div>
        </div>);
    ...
    const ModulesAndCourses = () => {
        ...
        const fetchModuleContent = (moduleId) => {
            const token = localStorage.getItem('token');
        ...
        if (error)
            return <p className="p-4 text-red-600">Error loading modules: {error.includes('404') ? 'Endpoint not found. Please check the server.' : error}. Try again later.</p>;
        if (courses.length === 0)
            return <p className="p-4 text-gray-600">Loading courses and modules...</p>;
        return (<div className="p-4 relative">
          ...
          <div className="space-y-6">
            ...
            }} className="p-2 border rounded w-full mb-2" value={selectedCourse?._id || ''}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {selectedCourse && (<div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100 hover:shadow-md transition-shadow duration-200">
              ...
              {modules.length > 0 ? (modules.map((module, index) => (<div key={index} className="bg-white p-4 rounded-lg shadow-sm border border-gray-100 hover:shadow-md transition-shadow duration-200">
                        ...
                        <button onClick={() => fetchModuleContent(module._id)} className="mt-2 bg-[#C9A500] text-white px-3 py-1 rounded hover:bg-[#A78A00] transition-colors duration-200">
                          View Module
                        </button>
                      </div>))) : (<p className="text-gray-600">No modules available for this course.</p>)}
            </div>
          </div>
          {isSideTabOpen && (<div className="fixed top-0 left-0 h-full w-1/3 bg-white shadow-lg z-50 transition-transform duration-300 transform translate-x-0">
          ...
          {isSideTabOpen && (<div className="fixed inset-0 bg-black bg-opacity-50 z-40" onClick={() => {
                    setIsSideTabOpen(false);
                    setModuleContent(null);
                }}/>)}
        </div>);
    };
    const KeyLinks = () => (<div className="p-4">
        <h2 className="text-lg font-semibold text-[#010D60] mb-4">Key Links</h2>
        <div className="space-y-2">
          <a href="https://lms-support.example.com" target="_blank" rel="noopener noreferrer" className="block p-4 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 text-[#010D60] hover:text-[#364CE2] border border-gray-100">
            <FaLink className="inline mr-2"/> LMS Support
          </a>
          <a href="https://digital-capabilities.example.com" target="_blank" rel="noopener noreferrer" className="block p-4 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 text-[#010D60] hover:text-[#364CE2] border border-gray-100">
            <FaLink className="inline mr-2"/> Digital Capabilities and Accessibility
          </a>
          <a href="https://students-union.example.com" target="_blank" rel="noopener noreferrer" className="block p-4 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 text-[#010D60] hover:text-[#364CE2] border border-gray-100">
            <FaLink className="inline mr-2"/> Students' Union
          </a>
          <a href="https://strategies-policies.example.com" target="_blank" rel="noopener noreferrer" className="block p-4 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 text-[#010D60] hover:text-[#364CE2] border border-gray-100">
            <FaLink className="inline mr-2"/> Strategies and Policies
          </a>
          <a href="https://data-protection.example.com" target="_blank" rel="noopener noreferrer" className="block p-4 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 text-[#010D60] hover:text-[#364CE2] border border-gray-100">
    ...
    return (<div className="flex h-screen bg-white font-inter">
        <div className={`${isSidebarOpen ? 'w-64' : 'w-20'} bg-gradient-to-b from-[#010D60] to-[#364CE2] text-white transition-all duration-300 ease-in-out fixed h-full z-10`}>
          <div className="p-4">
          ...
          <nav className="mt-4">
            <ul>
              <li className="mb-2">
                <Link to="/key-links" className="flex items-center p-2 hover:bg-[#C9A500] rounded text-white hover:text-[#010D60]">
                  <FaLink className="mr-2"/>
                  {isSidebarOpen && <span>Key Links</span>}
                </Link>
              </li>
              <li className="mb-2">
                <Link to="/student-profile" className="flex items-center p-2 hover:bg-[#C9A500] rounded text-white hover:text-[#010D60]">
                  <FaUser className="mr-2"/>
                  {isSidebarOpen && <span>Student Profile</span>}
                </Link>
              </li>
              <li className="mb-2">
                <Link to="/activity" className="flex items-center p-2 hover:bg-[#C9A500] rounded text-white hover:text-[#010D60]">
                  <FaCalendarAlt className="mr-2"/>
                  {isSidebarOpen && <span>Activity</span>}
                </Link>
              </li>
              <li className="mb-2">
                <Link to="/modules-courses" className="flex items-center p-2 hover:bg-[#C9A500] rounded text-white hover:text-[#010D60]">
                  <FaBook className="mr-2"/>
                  {isSidebarOpen && <span>Modules and Courses</span>}
                </Link>
              </li>
              <li className="mb-2">
                <Link to="/academic-calendar" className="flex items-center p-2 hover:bg-[#C9A500] rounded text-white hover:text-[#010D60]">
                  <FaCalendarAlt className="mr-2"/>
                  {isSidebarOpen && <span>Academic Calendar</span>}
                </Link>
              </li>
              <li className="mb-2">
                <Link to="/messages" className="flex items-center p-2 hover:bg-[#C9A500] rounded text-white hover:text-[#010D60]">
                  <FaEnvelope className="mr-2"/>
                  {isSidebarOpen && <span>Messages</span>}
                </Link>
              </li>
              <li className="mb-2">
                <Link to="/marks" className="flex items-center p-2 hover:bg-[#C9A500] rounded text-white hover:text-[#010D60]">
                  <FaChartBar className="mr-2"/>
                  {isSidebarOpen && <span>Marks</span>}
                </Link>
              </li>
              <li className="mb-2">
                <Link to="/tools" className="flex items-center p-2 hover:bg-[#C9A500] rounded text-white hover:text-[#010D60]">
                  <FaTools className="mr-2"/>
                  {isSidebarOpen && <span>Tools</span>}
                </Link>
              </li>
              <li className="mb-2">
                <button onClick={handleSignOut} className="flex items-center w-full p-2 hover:bg-[#C9A500] rounded text-white hover:text-[#010D60]">
          ...
          <div className="p-6">
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white p-4 rounded-lg shadow-md">
                ...
                {!loading && (<div className="max-w-[90vw] xs:max-w-[92vw] sm:max-w-[1000px] mx-auto">
