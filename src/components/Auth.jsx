...
const Auth = () => {
    ...
    const validateUniqueID = (id) => /^[a-zA-Z0-9]{6,}$/.test(id.trim());
    ...
    const handleRegisterSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(`${BASE_URL}/api/auth/register`, { fullName, email, password: password.trim(), phoneNumber }, { headers: { 'Content-Type': 'application/json' }, timeout: 15000 });
            const { token, user } = response.data;
            localStorage.setItem('token', token);
            setUser(user);
            navigate('/userdashboard');
    ...
    return (<div className="min-h-screen w-full flex items-center justify-center bg-[url('./assets/Log_Background.jpeg')] bg-no-repeat bg-cover bg-center px-4 py-10 sm:px-6 lg:px-8">
        <div className="w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl bg-white/30 shadow-xl rounded-[20px] p-6 sm:p-8 backdrop-blur-2xl border border-white/40">
          {isRightPanelActive ? (<form onSubmit={handleRegisterSubmit} className="flex flex-col gap-4">
              ...
              </form>) : (<form onSubmit={handleLoginSubmit} className="flex flex-col gap-4">
                <h2 className="text-2xl sm:text-3xl font-bold text-center">Sign In</h2>
                {error && <p className="text-red-600 text-sm">{error}</p>}
                {isAdminLogin ? (<input type="text" placeholder="Unique ID" className="p-3 rounded-md border" value={loginUniqueID} onChange={(e) => setLoginUniqueID(e.target.value)} required/>) : (<input type="email" placeholder="Email" className="p-3 rounded-md border" value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} required/>)}
